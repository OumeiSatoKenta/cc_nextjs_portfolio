...
プロジェクトタイプを判定し、対応する検証コマンドを実行する。

### プロジェクトタイプの判定

- `*.tf` ファイルが存在する場合 → **IaC プロジェクト**
- `package.json` が存在する場合 → **Webアプリ**
- どちらでもない場合 → **その他**

### IaC プロジェクトの場合

```bash
terraform fmt -check   # フォーマット違反の検出
terraform validate     # 構文・参照の検証
tflint                 # 静的解析
```

> **注意:** `terraform plan` は AWS 認証が必要なため自動実行しない。
> 実行が必要な場合はユーザーに `aws-vault exec <profile> -- terraform plan` を案内する。
...
`Makefile` または `README.md` を読み、検証コマンドを特定して実行する。
コマンドが見つからない場合はその旨をユーザーに報告してステップ8に進む。
