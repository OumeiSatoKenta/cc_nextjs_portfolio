...
1. 今日の日付を `YYYYMMDD` 形式で取得し、`[生成日]` 変数として保持する（例: `20260412`）。
2. `docs/ideas/` 内のマークダウンファイルを全て読む。
3. 内容を理解し、PRD作成の参考にする。

### ステップ1: プロダクト要求定義書の作成

1. **prd-writingスキル**をロード
2. `docs/ideas/`の内容を元に`docs/product-requirements.md`を作成
   - ファイル先頭に `<!-- 生成日: [生成日] -->` を挿入する
3. 壁打ちで出たアイデアを具体化：
   - 詳細なユーザーストーリー
   - 受け入れ条件
   - 非機能要件
...
### ステップ7: next-steps.md の自動生成

6ドキュメント作成完了後、`.steering/[生成日]-setup/next-steps.md` を自動生成する。

内容:
1. **作成したドキュメント一覧** — 各ファイルパスと一言説明
2. **次の `/add-feature` 候補** — PRD のユーザーストーリーから導出した機能実装コマンド例（3〜5個）
3. **プロジェクトタイプ別の補足情報**
   - IaC（`*.tf` ファイルが想定される場合）: `terraform init` / `aws-vault` のセットアップ案内
   - Webアプリ（`package.json` が想定される場合）: `npm install` / 環境変数設定の案内

## 完了条件

- 6つの永続ドキュメントが全て作成されていること
- `.steering/[生成日]-setup/next-steps.md` が生成されていること
