...
| 技術 | バージョン | 用途 | 選定理由 |
|------|-----------|------|----------|
| Next.js | 14+ | Webフレームワーク（App Router + Static Export） | Static Export対応、App Routerによるレイアウト共有、`next/font`によるGeistフォント標準搭載 |
| React | 18+ | UIライブラリ | Next.jsに内包。コンポーネントベースのUI構築 |
| Tailwind CSS | 3.x | スタイリング | ユーティリティベースでDESIGN.mdデザイントークンとの統合が容易。ビルド時に未使用CSSを除去 |
...
| 技術 | バージョン | 用途 | 選定理由 |
|------|-----------|------|----------|
| ESLint | 9.x | 静的解析 | コード品質の担保。Next.js推奨設定をベースに使用 |
| Prettier | 3.x | コードフォーマッター | コーディングスタイルの統一 |
| Jest | — | ユニットテスト | React Testing Libraryとの統合が成熟 |
...
| デザイントークン | `DESIGN.md` + `tailwind.config.ts` | Markdown + TypeScript | DESIGN.mdが真実の源泉、Tailwindが実行時制約 |
...
| Tailwind CSS Purge | スタイルシート | 未使用CSSの自動除去。最終CSSサイズの最小化 |
...
| ライブラリ | 用途 | バージョン管理方針 |
|-----------|------|-------------------|
| next | Webフレームワーク | `^14.0.0`（メジャーバージョン内で自動更新） |
| react / react-dom | UIライブラリ | Next.jsの要求バージョンに合わせる |
...
| eslint | 静的解析 | `^9.0.0` |
| prettier | フォーマッター | `^3.0.0` |
| jest | テストフレームワーク | `^29.0.0` |
| @testing-library/react | コンポーネントテスト | `^14.0.0` |
