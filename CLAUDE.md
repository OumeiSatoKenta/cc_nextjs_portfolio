...
このプロジェクトはプロジェクトルートの `DESIGN.md` にデザインシステムを定義している。
UIコンポーネントの生成・修正時は必ずこのファイルを参照すること。

- `DESIGN.md` に定義されたカラー、フォント、スペーシング値のみを使用する
- フレームワークのデフォルト値や独自の値を発明しない
- コンポーネントの状態（hover, focus, active等）は `DESIGN.md` のパターンに従う
- Tailwind CSS の設定ファイル (`tailwind.config.ts`) にデザイントークンを反映し、二重の制約レイヤーとする

## ポートフォリオコンテンツの一次ソース

このポートフォリオサイトに掲載する**本人に関するコンテンツ**（プロフィール、スキル一覧、職務経歴、プロジェクト実績、自己 PR、コミュニティ活動等）は、必ず以下を**一次ソース**として参照すること。

- **PDF 本体**: `docs/20260411_resume.pdf`（`.gitignore` 済 / ローカル参照のみ）
- **要点サマリ**: auto-memory の `project_resume_facts.md`（毎セッションで自動ロード）

### 適用範囲

About / Profile / Projects / Skills / Experience など、本人情報を表示・記述するすべての機能・ページ・コンポーネント・コピーライティング。

### ルール

1. **事実は PDF / メモリから**: 会社名・期間・技術スタック・スキルレベル・年数等の具体値を発明・推測しない。
2. **差分がある時は PDF が正**: メモリ要点と PDF に食い違いがあれば PDF を信頼し、メモリを更新する。
3. **コミット禁止**: PDF 本体および個人情報（氏名・生年・最寄駅等）をリポジトリ内のファイル（ソースコード・ドキュメント・コミットメッセージ）に**直接書き込まない**。ポートフォリオサイトに公開する情報は PDF の自己 PR / 職務要約レベルに留める、もしくはユーザーに都度確認する。
4. **迷ったら確認**: PDF に記載のない情報（例: 趣味・顔写真）が必要になった場合は、ユーザーに確認してから追加する。
...
3. `/add-feature-planning [機能]` で段階計画を作成(大規模機能の場合)
...
# 大きな機能の段階計画(計画書 + コマンド一覧を生成)
> /add-feature-planning ユーザープロフィール機能
